...
public class Player : MonoBehaviour {
    ...
    [SerializeField]
    private Rigidbody2D
        rb; // reference to the rigidbody so we can apply velocity to it, make sure to drag it in the scene!

    [SerializeField] private TMP_Text text; // score ui text object. make sure to drag it in the scene!
    public int score = 0;

    [SerializeField]
    private GameObject noteTextDialogue; // reference to the note dialogue, make sure to drag it in the scene!

    [SerializeField]
    private GameObject hippieTextDialogue; // reference to the hippie dialogue, make sure to drag it in the scene!

    private bool
        canPortal = false; // only if this is true, the player can use the portal. this will be set to true after hitting the note.
    ...
    [SerializeField] private Transform
        teleportDestination; // reference to the teleport destination so we can teleport the player to the center of the world

    [SerializeField] private Sprite happyKid;
    ...
    private void Update() {
        ...
        float x = Input.GetAxisRaw("Horizontal"); // A or D
        float y = Input.GetAxisRaw("Vertical"); // W or S
    ...
    private void OnCollisionEnter2D(Collision2D other) {
        ...
        if (other.gameObject.CompareTag("Pineapple")) {
            score++; // score = score + 1
            text.SetText("Pineapple: " + score); // update ui text 

            // destroy the pineapple, despawn
    ...
    private void OnTriggerEnter2D(Collider2D other) {
        ...
        if (other.gameObject.CompareTag("Note")) {
            ...
            canPortal = true;
            portal.gameObject.SetActive(true); // activate the portal
        }
        ...
        if (other.gameObject.CompareTag("Kid")) {
            if (score == 0) return; // stop the kid from becoming happy if we dont have any pineapples.

            score = 0;
            text.SetText("Pineapple: " + score);
            other.gameObject.GetComponent<SpriteRenderer>().sprite = happyKid;
        }
    }
